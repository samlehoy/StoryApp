package com.bangkit.storyapp.data.paging
...
import com.bangkit.storyapp.data.database.RemoteKeys
    ...
    override suspend fun load(
        ...
        try {
            ...
            database.withTransaction {
                ...
                val keys = response.listStory?.filterNotNull()?.map { story ->
                    RemoteKeys(id = story.id, prevKey = prevKey, nextKey = nextKey)
                } ?: emptyList()
                database.remoteKeysDao().insertAll(keys)
                database.storyDao().insertStories(response.listStory?.filterNotNull() ?: emptyList())
            }
